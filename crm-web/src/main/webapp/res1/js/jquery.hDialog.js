...
;(function($, window, document, undefined) {	
	...
	methods = {
        ...
        add: function (o,$this) { //显示弹框
			...
			w = o.width != undefined ? parseInt(o.width) : '700';
			h = o.height != undefined ? parseInt(o.height) : '600';
			m = ""+(-(h/2))+'px 0 0 '+(-(w/2))+"px";
			...
			if(o.title != ''){ $obj.stop().prepend('<div id="HTitle">'+o.title+'</div>'); }
			if(o.closeHide != false){
				$obj.stop().append('<a id="HCloseBtn" title="关闭" style="width:24px;height:24px;line-height:18px;display:inline-block;cursor:pointer;background-color:'+closeBg+';color:#ccc;font-size:1.4em;text-align:center;position:absolute;top:8px;right:8px;"><span style="margin-top:3px;width:24px;height:24px;display:inline-block;">×</span></a>').css({'position':'fixed','backgroundColor':o.boxBg,'top':t,'left':l,'margin':m,'zIndex':'100000'});
			}
			$obj.stop().animate({'width':o.width,'height':o.height},600).removeAttr('class').addClass('animated').show();
			//$obj.stop().animate({'width':o.width,'height':o.height},600).removeAttr('class').addClass('animated'+c+'').show();
        },
	...
	*/
	$.extend({  
		 ...
		 * @param t : string 加载文字
		 * @param cancelCB : function 取消回调方法
		 */
	    showLoading: function (t, cancelCB) { //显示加载
        	t = t != undefined ? t : '正在加载...';
        	var margin = "-60px 0 0 -50px";
        	methods.remove('#HLoading');
        	$body.stop().append("<div id='HLoading' style='width:"+_doc.width()+"px;height:"+_doc.height()+"px;background-color:rgba(0,0,0,0.5);position:fixed;top:0;left:0;z-index:9999;'>" +
        			"<div id='HLoadingDiv' style='line-height:30px;color:#fff;text-align:center;position:fixed;top:50%;left:50%;margin:"+margin+";'><div><img src='../../res/img/loading.gif'></div>"+t+"</div></div>");
        	if (typeof cancelCB == 'function') {
        		$('#HLoadingDiv').append("<span id='loadingCancel' style='cursor:pointer'>取消</span>");
