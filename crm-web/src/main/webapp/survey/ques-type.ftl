...
<script src="../js/common-date.js" charset="utf-8"></script>
      ...
      //时间戳的处理
        ...
        var tableIns = table.render({
            ...
            , cols: [[                  //标题栏
                ...
                , {field: 'quesTypeId', title: '题目大类ID', width: 100}
        ...
        table.on('tool(dateTableId)', function(obj){ //注：tool 是工具条事件名，dateTableId 是 table 原始容器的属性 lay-filter="对应的值"
          ...
          if(layEvent === 'detail'){
            layer.msg('查看操作');
        	  ...
        	  //layer.msg('添加操作');
              	var index = layer.open({
              		  ...
              		  fixed: false, //不固定
              		  maxmin: true,
              		  content: '/crm-web/getQuesFtl?quesSurveyId=' + data.quesSurveyId + '&quesTypeId=' + data.quesTypeId
              	});
              	layer.full(index);
            ...
            layer.confirm('真的删除行么', function(index){
              ...
              layer.close(index);
              //向服务端发送删除指令
              asyncXhr2('/crm-web/deleteQuesType.ftl?quesTypeId='+data.quesTypeId, '', "POST", 'application/json', function(data){
            ...
            //layer.msg('编辑操作');
	            var index = layer.open({
	        		  ...
	        		  fixed: false, //不固定
	        		  maxmin: true,
	        		  content: '/crm-web/getQuesTypeFtl.ftl?quesSurveyId=' + data.quesSurveyId + '&quesTypeId=' + data.quesTypeId
	        	});
	            layer.full(index);
          }
        });
...
<script type="text/html" id="barOption">
    <a class="layui-btn layui-btn-mini layui-btn-normal" lay-event="add">添加题目</a>
