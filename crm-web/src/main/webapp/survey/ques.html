        ...
        table.on('tool(dateTableId)', function(obj){ //注：tool 是工具条事件名，dateTableId 是 table 原始容器的属性 lay-filter="对应的值"
          ...
          if(layEvent === 'detail'){
            layer.msg('查看操作');
        	  ...
        	  //layer.msg('添加操作');
              	layer.open({
              		  ...
              		  fixed: false, //不固定
              		  maxmin: true,
              		  content: '/crm-web/getQuesAnswerFtl?quesId=' + data.quesId
              	});
            ...
            layer.confirm('真的删除行么', function(index){
              ...
              layer.close(index);
              //向服务端发送删除指令
              asyncXhr2('/crm-web/deleteQues.ftl?quesId='+data.quesId, '', "POST", 'application/json', function(data){
	            ...
	            var index = layer.open({
	        		  ...
	        		  fixed: false, //不固定
	        		  maxmin: true,
	        		  content: '/crm-web/getQues.ftl?quesSurveyId=' + data.quesSurveyId + '&quesId='+data.quesId
	        	});
	            layer.full(index);
          }
        });
